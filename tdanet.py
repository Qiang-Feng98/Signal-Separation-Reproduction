...
import math
"""
Modified from JusperLee/TDANet: https://github.com/JusperLee/TDANet
"""
...
class TDANet(nn.Module):
    def __init__(self, input_dim, out_dim, features, num_blocks, upsampling_depth, enc_kernel_size):
        ...
        :param features: number of hidden features
        :param num_blocks: number of blocks
        :param upsampling_depth: upsampling depth
        :param enc_kernel_size: kernel size of encoder
    ...
    @staticmethod
    def pad_input(input, window, stride):
        """
        Zero-padding input according to window/stride size.
        """
        batch_size, channel, nsample = input.shape

        # pad the signals at the end for matching the window/stride size
        rest = window - (stride + nsample % window) % window
        if rest > 0:
            pad = torch.zeros(batch_size, channel, rest).type(input.type())
            input = torch.cat([input, pad], 2)
        pad_aux = torch.zeros(batch_size, channel, window - stride).type(input.type())
    ...
    def forward(self, input):
        ...
        x, rest = self.pad_input(
            input, self.enc_kernel_size, self.enc_kernel_size // 4
        )
        # Front end
        x = self.encoder(x)
        # Split paths
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        output, _ = self.attn(output, output, output)  # [T, B, C]
